...
object Task4HotelsApplication {
  ...
  /**
    ...
    */
  def loadDataset(spark: SparkSession, path: String): Dataset[Booking] = {
...
//    import com.github.mrpowers.spark.daria.sql.ColumnExt._
    ...
    df.show()
    df.printSchema()
//    df.createOrReplaceTempView("tempTrainParquet")

//    val filterCond = df.columns.map(x=>col(x).isNotNullOrBlank).reduce(_ && _)
    df
//      .filter(filterCond)
        .selectExpr(
          "CAST(is_booking as int) as is_booking",
          "CAST(srch_adults_cnt as int) as srch_adults_cnt",
          "CAST(srch_children_cnt as int) as srch_children_cnt",
          "CAST(user_location_country as int) as user_location_country",
          "CAST(hotel_continent as int) as hotel_continent",
          "CAST(hotel_country as int) as hotel_country",
          "CAST(hotel_market as int) as hotel_market"
        )
